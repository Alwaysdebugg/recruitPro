...
export const getHiredCandidates = async () => {
  const response = await axios.get<Candidate[]>(`${API_URL}/candidates/hired`);
  return response.data;
};

// 获取活跃面试列表
export const getActiveInterviews = async () => {
  const response = await axios.get<Interview[]>(`${API_URL}/interviews/active`);
  return response.data;
};

// 获取招聘进度中的候选人列表
export const getCandidatesInProgress = async () => {
  const response = await axios.get<Candidate[]>(`${API_URL}/candidates/in-progress`);
  return response.data;
};
